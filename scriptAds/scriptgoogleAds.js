function main() {
  ...
  function reporte(id){
    ...
    while(grupoCapana.hasNext()){
      ...
      var urlTemplate = "https://us-central1-cinebol2019.cloudfunctions.net/add?idCuenta=%idCuenta%&idCamapana=%idCamapana%&name=%name%&averageCpc=%averageCpc%&averageCpm=%averageCpm%&clicks=%clicks%&impresiones=%impresiones%&costo=%costo%&ctr=%ctr%&fecha=%fecha%";
           var url=urlTemplate.replace("%idCuenta%", encodeURIComponent(id))
                      .replace("%idCamapana%", encodeURIComponent(tipCuenta.getId()))
